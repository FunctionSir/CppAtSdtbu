 ...
 * @LastEditTime: 2025-03-20 15:14:39
 * @LastEditors: FunctionSir
 * @Description: Contacts management.
 * @FilePath: /CppAtSdtbu/1-Contacts/main.cpp
 */
...
void create_if_not_exists(string path) {
    if (path == "") {
...
inline void mod_menu(void) {
    ...
    cerr << "Tips: 您可以使用查找工具来获得索引值." << endl;
INPUT:
    cerr << "请输入要编辑的条目的索引(输入负值来取消): ";
    if (!(cin >> targetIndex) || targetIndex < 0) {
        cerr << "操作已取消." << endl;
        return;
    }
    if (size_t(targetIndex) >= book->size()) {
        cerr << "错误: 索引不合法" << endl;
        goto INPUT;
    }
    const Entry &ref = book->get_entry_const_ref(size_t(targetIndex));
    cerr << "=== [ 通讯录条目修改菜单 ] ===" << endl
         << "选中条目: " << targetIndex << endl
         << "[F] 修改名字" << endl
         << "[L] 修改姓氏" << endl
         << "[P] 修改手机号" << endl
         << "[T] 修改座机号" << endl
    ...
    case 'F':
        ...
        getline(cin, _);
        getline(cin, target);
        book->mod_entry(size_t(targetIndex), op, target);
        break;
    case 'L':
    ...
    NEW_PHONE:
        ...
        getline(cin, _);
        getline(cin, target);
        if (!entries::Entry::chk_phone(target)) {
            cerr << "您的输入有问题, 请重新输入.";
            goto NEW_PHONE;
        }
        book->mod_entry(size_t(targetIndex), op, target);
    ...
    NEW_TEL:
        ...
        getline(cin, _);
        getline(cin, target);
        if (!entries::Entry::chk_tel(target)) {
            cerr << "您的输入有问题, 请重新输入.";
            goto NEW_TEL;
        }
        book->mod_entry(size_t(targetIndex), op, target);
    ...
    NEW_EMAIL:
        ...
        getline(cin, _);
        getline(cin, target);
        if (!entries::Entry::chk_email(target)) {
            cerr << "您的输入有问题, 请重新输入.";
            goto NEW_EMAIL;
        }
        book->mod_entry(size_t(targetIndex), op, target);
...
int main(int argc, char *argv[]) {
    ...
    while (true) {
        ...
        case 'D':
            long long tmp;
        INPUT_D:
            cerr << "请输入要删除的条目的索引(输入负值来取消): ";
            if (!(cin >> tmp) || tmp < 0) {
                cerr << "操作已取消." << endl;
                break;
            }
            if (size_t(tmp) >= book->size()) {
                cerr << "错误: 索引不合法" << endl;
                goto INPUT_D;
            }
