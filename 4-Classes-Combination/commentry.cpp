 ...
 * @Author: FunctionSir
 * @License: AGPLv3
 * @Date: 2025-05-14 21:49:41
 * @LastEditTime: 2025-05-15 14:22:57
 * @LastEditors: FunctionSir
 * @Description: -
 * @FilePath: /4-Classes-Combination/commentry.cpp
 */
...
size_t CommEntry::get_tot_entries_cnt(void) {
    return m_SizeTotEntriesExists;
}
size_t CommEntry::m_SizeTotEntriesExists = 0;
std::string CommEntry::to_csv(void) const {
    stringstream ss;
    ss << get_name();
    for (size_t i = 0; i < size(); i++) {
        ss << "," << m_StrPtrTypes[i] << "," << m_StrPtrNumbers[i];
    }
    return ss.str();
}
